...
public class CharacterEntity extends PanacheEntityBase {
    ...
    @Column(name = "stroke_order_json", columnDefinition = "JSON")
    public String strokeOrderJson;

    public static List<CharacterEntity> findByLevel(String level) {
        return list("jlptLevel = ?1 or hskLevel = ?1 order by frequencyRank, characterText", level);
    }
